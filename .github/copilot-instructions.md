...
### Users
メインのターゲットは、5chの「高温大学駅伝」スレッドに集まる、10年以上この企画に関わってきた理系で知的なニッチなファン層です。テキストベースの実況をより視覚的に楽しむためのツールを求めています。速報という性質上「見やすさ」を担保しつつ、レース特有の「緊迫感」や「熱狂」を感じられるインターフェースが目標です。

### Brand Personality
ブランドの性格は「分析的」「ミニマル」「テクニカル」。
日本テレビ（NTV）の箱根駅伝特設サイトが主なリファレンスです。理系的なデータの緻密さと、スポーツの感情的な熱さを両立させるデザインを目指します。

### Aesthetic Direction
「ミニマル＆テクニカル」な美しさを基調とします。利用シーンとしては昼休みや日中の閲覧も多いため、OSのシステム設定（ライト/ダーク）に追従する柔軟なテーマ設計とします。また、ダークモード時に「完全な黒（#000000）」を使用することは避け、目に優しいダークグレー系の配色を採用します。

### Design Principles
1. **モバイルファーストの情報整理**：ユーザーの大半がスマートフォンから閲覧するため、膨大な情報量（各区間、成績、監督コメントなど）を小さい画面でも適切に取捨選択・整理して表示する。
2. **分析的かつスリリングなUI**：NTVの箱根駅伝サイトを参考に、数字（気温＝距離）の羅列だけに終わらず、リアルタイムの成績推移による緊迫感を演出する。
3. **テクニカルでノイズの少ないデザイン**：理知的でディープなファンのために、過剰な装飾は省き、データそのものが主役となるミニマルな外観を維持する。
4. **システムに馴染むテーマと視認性**：昼夜問わず閲覧されることを前提に、OSのカラーテーマに自然に追従すること。ダークモード実装時は完全なブラックを避け、上品で視認性の高い配色設計を意識する。
