...
if ! git diff --quiet --exit-code realtime_report.json data/individual_results.json rank_history.json leg_rank_history.json runner_locations.json; then
    echo "速報ファイル (realtime_report.json, etc.) に変更を検出しました。GitHubにプッシュします。"
    git add realtime_report.json data/individual_results.json rank_history.json leg_rank_history.json runner_locations.json
