...
# OpenAI Structured Outputs用JSON Schema（strict mode対応、全フィールドrequired/null許容）
    ...
    def build_user_prompt(self, metrics):
        ...
        # 終盤（走行中チームが少数）は記事を短縮する（ユーザー指示 2026-08-25）。
        # active_sorted が空（全ゴール後・データ未整備時）は従来どおり通常モード。
        active_team_count = len(active_sorted)
        if 0 < active_team_count <= 3:
            total_length_rule = (
                f"- 全体の文字数は約800文字（600〜1,000字、記事全体）。走行中チームは残り{active_team_count}校のみの終盤のため、簡潔にまとめること。\n"
            )
            headline_count_rule = (
                "- 2行目以降は、メイン見出し（『### 』）を1つ、その下に小見出し（『#### 』）は1〜2つまでとする（箇条書きは多用しない）。大きな争点が少ない終盤のため、件数を水増ししないこと。\n"
            )
            subhead_variability_rule = (
                "- 小見出しの数は1〜2件で可変とする（最後は必ず総括を含める）。件数を満たすための水増しは禁止する。\n"
            )
            topic_length_rule = "- 各トピック本文の文字数は200〜300字程度を目安とすること。\n"
        else:
            total_length_rule = "- 全体の文字数は約2,000文字（1,800〜2,200字、記事全体）。小見出し数に応じて可変。\n"
            headline_count_rule = "- 2行目以降は、レース全体を象徴するメイン見出し（『### 』）を1つ、その下に各トピックの小見出し（『#### 』）を通常2〜4つ書くこと（箇条書きは多用しない）。下位チームの注目候補がある場合のみ、専用セクションを1つ追加して最大5つまで可とする。\n"
            subhead_variability_rule = "- 小見出しの数は通常2〜4件で可変とする（大きな争点が少ない日は2件、複数の独立した争点がある日は3〜4件）。下位専用セクションを追加する場合は、他のトピックより短い150〜300字程度でよい。件数を満たすための水増しは禁止する。\n"
            topic_length_rule = "- 各トピック本文の文字数は400〜500字程度を目安とすること。\n"
