...
# Momento for history cache
try:
    from momento import CacheClient, Configurations, CredentialProvider
    from momento.responses import (
        CacheListFetch, CacheListPushFront, CreateCache
    )
    MOMENTO_INSTALLED = True
except ImportError:
    MOMENTO_INSTALLED = False
...
class DailySummaryGenerator:
    ...
    def __init__(self, dry_run=False):
        ...
        self.all_data = {}
        self.momento_client = None
        self.gemini_model = None

        load_dotenv()

        self.cache_name = os.getenv("MOMENTO_CACHE_NAME")
        self.article_history_key = os.getenv("MOMENTO_HISTORY_KEY", "high-daigaku-ekiden")

        self._setup_clients()

    def _setup_clients(self):
        """Initializes Gemini and Momento API clients."""
        # --- Gemini Setup ---
        if not self.dry_run:
        ...
        # --- Momento Setup ---
        if not MOMENTO_INSTALLED:
            print("情報: 'momento' ライブラリがインストールされていません。履歴機能は無効になります。`pip install momento`でインストールしてください。")
            return

        momento_api_key = os.getenv("MOMENTO_API_KEY")
        if not self.cache_name or not momento_api_key:
            print("情報: Momentoの環境変数 (MOMENTO_API_KEY, MOMENTO_CACHE_NAME) が未設定のため、記事履歴機能は無効になります。")
            return

        try:
            self.momento_client = CacheClient(
                Configurations.Laptop.v1(),
                CredentialProvider.from_string(momento_api_key),
                default_ttl=timedelta(days=30)
            )
            create_cache_response = self.momento_client.create_cache(self.cache_name)
            if isinstance(create_cache_response, (CreateCache.Success, CreateCache.CacheAlreadyExists)):
                print(f"✅ Momento Cache '{self.cache_name}' に接続しました。")
            elif isinstance(create_cache_response, CreateCache.Error):
                print(f"❌ Momento Cacheの作成/接続に失敗しました: {create_cache_response.message}")
                self.momento_client = None
        except Exception as e:
            print(f"❌ Momento Cacheへの接続中に予期せぬエラーが発生しました: {e}")
            self.momento_client = None

    def _get_article_history(self, num_articles=2):
        """Fetches the last N articles from Momento Cache."""
        if not self.momento_client:
            return []

        print(f"Momento Cacheから過去の記事履歴を取得しています (key: {self.article_history_key})...")
        response = self.momento_client.list_fetch(self.cache_name, self.article_history_key)

        if isinstance(response, CacheListFetch.Hit):
            articles = response.value_list_string
            print(f"✅ {len(articles)}件の記事履歴を発見。最新の{num_articles}件を利用します。")
            return articles[:num_articles]
        elif isinstance(response, CacheListFetch.Miss):
            print("情報: 記事履歴は見つかりませんでした。")
            return []
        elif isinstance(response, CacheListFetch.Error):
            print(f"❌ 記事履歴の取得中にエラーが発生しました: {response.message}")
            return []
        return []

    def _save_article_to_history(self, article_text):
        """Saves the newly generated article to the history in Momento Cache."""
        if not self.momento_client or not article_text:
            return

        print(f"Momento Cacheに新しい記事を保存しています (key: {self.article_history_key})...")
        response = self.momento_client.list_push_front(
            self.cache_name, self.article_history_key, article_text,
            truncate_back_to_size=10
        )
        if isinstance(response, CacheListPushFront.Success):
            print("✅ 新しい記事を履歴に保存しました。")
        elif isinstance(response, CacheListPushFront.Error):
            print(f"❌ 記事履歴の保存中にエラーが発生しました: {response.message}")
    ...
    def _build_prompt(self):
        ...
        prompt_parts = [
            ...
            "- **監督コメントの活用**: **【昨晩の監督コメント】にデータがある場合のみ**、その内容を引用または言及してください。監督の言葉が現実になったのか（例：「信じていた」→快走）、ならなかったのか（例：「不安が的中」→苦戦）を関連付けて描写し、物語に深みを与えてください。**コメントがない場合は、監督の発言に一切触れないでください。**",
            ...
            "- **架空の情報の創作**: 提供されたデータにない、架空のコメントやエピソード（例：「監督は涙を流した」など）は絶対に創作しないでください。**特に、提供されていない監督コメントを文脈から推測して創作することは固く禁じます。**",
            "- **単調なデータの羅列**: 「A大学はXkm、B大学はYkmでした」のような、単なるデータの読み上げは避けてください。必ずあなたの解説と物語を加えてください。"
        ]

        article_history = self._get_article_history(num_articles=2)
        if article_history:
            history_section = ["\n## 【これまでのレース展開（過去のあなたの解説）】", "過去の解説を踏まえて、今日のレース展開の連続性や特筆すべき変化を解説に含めてください。"]
            for i, article in enumerate(article_history, 1):
                day = race_day - i
                if day > 0:
    ...
    def run(self):
        ...
        try:
            ...
            self._save_article_to_history(raw_article_text)
        except Exception as e:
