...
def test_validation_noun_exclusion():
    ...
    gen = setup_mock_generator()
    metrics = gen.calculate_race_metrics()
    # ラッパー: validate_generated_article は (can_save, warnings, fatal_errors) を返す
    def v(article_text, metrics):
        can_save, _, _ = gen.validate_generated_article(article_text, metrics)
        return can_save
    ...
    assert v(article_good, metrics), "Error: 正常な一般名詞の混ざった記事がバリデーション失敗しました"

    # 2. 太字なしの正常表現の通過確認
    article_good_no_bold = "名古屋大学の美濃加茂君が快走。"
    assert v(article_good_no_bold, metrics), "Error: 太字なしの正常表現がバリデーション失敗しました"

    # 2.5 接頭辞（「位」「首位」）付きの正常大学名の通過確認
    article_prefix_uni1 = "1位名古屋大学が激走。"
    assert v(article_prefix_uni1, metrics), "Error: 『1位名古屋大学』がバリデーション失敗しました"
    article_prefix_uni2 = "首位名古屋大学がリード。"
    assert v(article_prefix_uni2, metrics), "Error: 『首位名古屋大学』がバリデーション失敗しました"

    # 3. 架架空大学名や架空選手名（太字なし・助詞境界あり）
    article_bad_uni = "架空大学が本日急浮上しました。"
    assert not v(article_bad_uni, metrics), "Error: 存在しない大学名（架空大学）がパスしてしまいました"

    article_bad_runner = "架空太郎君が快走を見せました。"
    assert not v(article_bad_runner, metrics), "Error: 存在しない選手名（架空太郎君）がパスしてしまいました"

    # 4. 太字で装飾された架空大学名や架空選手名
    article_bad_uni_bold = "**架空大学**が本日急浮上しました。"
    assert not v(article_bad_uni_bold, metrics), "Error: 太字付きの存在しない大学名（**架空大学**）がパスしてしまいました"

    article_bad_runner_bold = "**架空太郎君**が快走しました。"
    assert not v(article_bad_runner_bold, metrics), "Error: 太字付きの存在しない選手名（**架空太郎君**）がパスしてしまいました"
...
def test_validation_winner_check():
    ...
    assert not v(article_win1, metrics), "Error: 未ゴール首位の優勝判定がパスしてしまいました"
    ...
    assert not v(article_win2, metrics), "Error: 首位以外の優勝記述がパスしてしまいました"

    # シナリオ3: 首位ゴール後に首位校を優勝と書いた場合（太字装飾あり） -> 成功
    article_win3 = "**名古屋大学**が今大会の優勝を決めた。"
    assert v(article_win3, metrics), "Error: 首位ゴール校の正当な優勝記述が失敗しました"
    ...
    assert v(article_win4, metrics), "Error: 過去の優勝に関する記述が誤検知で失敗しました"

    # 【追加テスト P1-1】
    # 1. 「第2日目、金沢大学が優勝を決めた」 -> 失敗
    article_win_p1_1 = "第2日目、**金沢大学**が優勝を決めた。"
    assert not v(article_win_p1_1, metrics), "Error: 第2日目の金沢大優勝（未ゴール）がパスしてしまいました"

    # 2. 「第2日目、名古屋大学が優勝を決めた」 -> 未ゴール失敗、1位ゴール後成功
    article_win_p1_2 = "第2日目、**名古屋大学**が優勝を決めた。"
    assert not v(article_win_p1_2, metrics), "Error: 第2日目の未ゴール名古屋大優勝がパスしてしまいました"
    
    # 1位ゴール後に設定
    gen.all_data["realtime_report"]["teams"][0]["runner"] = "ゴール"
    gen.all_data["realtime_report"]["teams"][0]["finishDay"] = 2
    assert v(article_win_p1_2, metrics), "Error: 第2日目のゴール後名古屋大優勝が失敗しました"
    ...
    assert v(article_win_p1_3, metrics), "Error: 過去の優勝実績記述が失敗しました"

    # 4. 「連覇経験を持つ名古屋大学」 -> 成功
    article_win_p1_4 = "連覇経験を持つ**名古屋大学**が好走。"
    assert v(article_win_p1_4, metrics), "Error: 連覇経験記述が失敗しました"

    # 5. 「3連覇となる優勝を決めた」 -> 1位ゴールなら成功、それ以外は失敗
    article_win_p1_5 = "3連覇となる優勝を決めたのは、**名古屋大学**だ。"
    assert not v(article_win_p1_5, metrics), "Error: 3連覇での未ゴール優勝がパスしました"
    
    gen.all_data["realtime_report"]["teams"][0]["runner"] = "ゴール"
    gen.all_data["realtime_report"]["teams"][0]["finishDay"] = 2
    assert v(article_win_p1_5, metrics), "Error: 3連覇でのゴール後優勝が失敗しました"
    ...
    assert v(article_win_p1_2_1, metrics), "Error: 複数校混在の正当な優勝判定が失敗しました"

    # 2. 1位ゴール後、「名古屋大学を追い金沢大学が優勝」 -> 失敗
    article_win_p1_2_2 = "**名古屋大学**を追い**金沢大学**が優勝した。"
    assert not v(article_win_p1_2_2, metrics), "Error: 複数校混在の誤った優勝判定がパスしてしまいました"

    # 3. 「優勝は名古屋大学」 -> 対象照合
    article_win_p1_2_3 = "今大会の優勝は**名古屋大学**だ。"
    assert v(article_win_p1_2_3, metrics), "Error: 『優勝はX』形式の正当な判定が失敗しました"
