...
if ! git diff --quiet --exit-code ekiden_state.json individual_results.json rank_history.json leg_rank_history.json runner_locations.json; then
    echo "状態ファイル (ekiden_state.json, etc.) に変更を検出しました。GitHubにプッシュします。"
    git add ekiden_state.json individual_results.json rank_history.json leg_rank_history.json runner_locations.json
